...
def initialGUI():
    ...
    window = tk.Tk()
    window.title(TITLE)
    bt1=tk.Button(text='入库单',command=lambda :click(1))
    bt1.pack()
    bt1 = tk.Button(text='出库单',command=lambda :click(0))
    bt1.pack()
    bt1 = tk.Button(text='移库单',command=lambda :click(0))
    def click(q):
        if q==0:
            ...
            #     j=result[0]
            #     print(i,j)
            #     # sql='insert into tingshi.in values '+()
    bt1.pack()
    window.mainloop()
# initialGUI()
